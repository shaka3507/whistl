...
import { AlertTriangle, Bell, ImageIcon, MessageSquare, Send, Users } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function ChannelPage() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (newMessage.trim().toLowerCase() === "acknowledge" && alert && !alertAcknowledged) {
  ...
  const handleInviteUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const successMessage = data.message || `Invitation sent to ${inviteEmail}`;
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      // Get public URL
      const { data: urlData } = supabase.storage.from("message-attachments").getPublicUrl(fileData.path)
  ...
  useEffect(() => {
    ...
    const checkAcknowledgment = async () => {
      try {
        ...
        if (data) {
          setAlertAcknowledged(true)
        } else if (typeof alert.requires_acknowledgment === 'boolean' && alert.requires_acknowledgment) {
        ...
        <div className="border-b">
          <div className="container flex items-center justify-between h-14 px-4">
            <div className="flex items-center gap-2">
              ...
              {alert && <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full">Active Alert</span>}
            </div>
            <div className="flex items-center gap-2">
              <Dialog>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>{members.length} members in this channel</DialogDescription>
                  </DialogHeader>
              ...
              {alert && (
                <Dialog>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle className="text-red-600">{alert.title}</DialogTitle>
                      <DialogDescription>{new Date(alert.created_at).toLocaleString()}</DialogDescription>
                    </DialogHeader>
                    <div className="mt-4">
              ...
              {messages.length === 0 ? (
                ...
                <div className="space-y-6">
                  {messages.map((message) => {
                    ...
                    const hasImage = message.content.includes("[Image]") && message.has_attachment
                    ...
                    if (hasImage) {
                      ...
                      if (urlMatch) {
                        imageUrl = urlMatch[0]
                        content = message.content.replace(imageUrl, "").replace("[Image]", "")
                      }
                    }
                    ...
                    return (
                      <div key={message.id} className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <div className="font-medium">{message.profiles.full_name}</div>
                            <div className="text-xs text-muted-foreground">
                              {new Date(message.created_at).toLocaleString()}
                            </div>
                          </div>
                ...
                <Tabs defaultValue="message" className="mb-4">
                  ...
                  <TabsContent value="notification">
                    <form onSubmit={handleSendNotification} className="flex gap-2">
                      ...
                      />
                      <Button type="submit" disabled={isSending || !notificationText.trim()} className="h-10">
                  ...
                  <TabsContent value="upload">
                    <div className="flex gap-2">
                      <Input type="file" accept="image/*" ref={fileInputRef} disabled={isSending} />
